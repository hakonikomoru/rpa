...
class GoogleAccountCreatePage(BasePage):
    def __init__(self, driver):
        url = 'https://accounts.google.com/signup/v2/webcreateaccount?continue=https%3A%2F%2Fmyaccount.google.com%3Futm_source%3Daccount-marketing-page%26utm_medium%3Dcreate-account-button&flowName=GlifWebSignIn&flowEntry=SignUp'
    ...
    def 名前アドレスパスワード入力(
        ...
        sleep(2)
        search = self.driver.find_element_by_name("lastName")
        search.send_keys(lastName)
        search = self.driver.find_element_by_name("firstName")
        ...
        search = self.driver.find_element_by_name("ConfirmPasswd")
        search.send_keys(confirmPasswd)
    ...
    def 電話番号を入力(
        ...
        sleep(2)
        search = self.driver.find_element_by_id("phoneNumberId")
        search.send_keys(phoneNumber)
        search.send_keys(Keys.ENTER)
