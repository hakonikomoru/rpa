...
twitterLoginPage.Twitterログイン()
amazonPage = MyAmazonPage(driver)
amazonPage.open()

for num in range(15):
    amazonPage.商品画面をURLで直接開く('https://www.amazon.co.jp/s?i=merchant-items&me=AO3JD7ELZ9RTY&page='+str(num+1))
    ...
    for itemsTag in itemsTags:
        try:
            if "/dp/" in itemsTag.get_attribute("href"):
                # ASINだけを抜く
                asins.append(itemsTag.get_attribute("href").split('/dp/')[1].split('/')[0])
        except:
            continue
