...
class MyAuctionPage(BasePage):
    ...
    def focusToElement(self, by, value, preventScroll):
        ...
        self.driver.execute_script(
            JavaScriptFocusToElement, element, preventScroll)
    ...
    def n行目の商品名を取得(self, n):
        selector = '#acWrContents > div > table > tbody > tr > td > table > tbody > tr:nth-child(3) > td > table:nth-child(6) > tbody > tr:nth-child('+str(
            n)+') > td:nth-child(3) > a'
    ...
    def 取引連絡ボタンを押下(self):
        ...
        count = len(self.driver.find_elements_by_css_selector(
            "#acWrContents > div > table > tbody > tr > td > table > tbody > tr:nth-child(2) > td > table:nth-child(2) > tbody > tr > td:nth-child(1) > a"))+1
        ...
        for pageCount in range(count):
            ...
            if pageNum > 1:
                self.driver.get(
                    'https://auctions.yahoo.co.jp/closeduser/jp/show/mystatus?select=closed&hasWinner=1&apg='+str(pageNum))
            for n in range(50):
                ...
                print(str(
                    num)+"行目------------------------------------------------------------------------------------")
                商品名 = '#acWrContents > div > table > tbody > tr > td > table > tbody > tr:nth-child(3) > td > table:nth-child(6) > tbody > tr:nth-child('+str(
                    num + 1)+') > td:nth-child(3) > a'
                print("商品名：" + self.driver.find_element_by_css_selector(商品名).text)
                # 取引連絡ボタンのselectorを生成
                selector = '#acWrContents > div > table > tbody > tr > td > table > tbody > tr:nth-child(3) > td > table:nth-child(6) > tbody > tr:nth-child('+str(
                    num + 1)+') > td:nth-child(8) > table > tbody > tr:nth-child(1) > td > a'
                # n個目の取引連絡ボタンを押下
                ...
                try:
                    ...
                    try:
                        rangeNum = self.driver.find_element_by_class_name(
                            "decTxt06").text.split('件中')[0]
                        print("落札件数　合計："+rangeNum+"件")
                        for n in range(int(rangeNum)):
                            num = n + 1
                            print(
                                "取引連絡の中の取引連絡"+str(num)+"行目の取引連絡----------------------------------------------------")
                            # あとで戻る可能性があるのでここでURLを取得しておく
                            curUrlSecond = self.driver.current_url
                            # 取引連絡ボタンのselectorを生成
                            selector = '#acWrContents > div > table > tbody > tr > td > table > tbody > tr > td > div:nth-child(5) > table > tbody > tr:nth-child('+str(
                                num + 1)+') > td:nth-child(5) > div.decBt01 > a'
                            try:
    ...
    def 評価ボタンを押下(self):
        ...
        for pageCount in range(count):
            ...
            for n in range(50):
                ...
                # 評価ボタンのselectorを生成
                selector = '#acWrContents > div > table > tbody > tr > td > table > tbody > tr:nth-child(3) > td > table:nth-child(6) > tbody > tr:nth-child('+str(
                    num + 1)+') > td:nth-child(8) > table > tbody > tr:nth-child(2) > td > a'
                print(str(
                    num)+"行目------------------------------------------------------------------------------------")
                # n個目の評価ボタンを押下
                self.driver.find_element_by_css_selector(selector).click()
                ...
                try:
                    ...
                    for n in range(int(rangeNum)):
                        num = n + 1
                        print(
                            "評価の中の評価"+str(num)+"行目の評価----------------------------------------------------")
                        # あとで戻る可能性があるのでここでURLを取得しておく
                        curUrlSecond = self.driver.current_url
                        # 評価ボタンのselectorを生成
                        selector = '#acWrContents > div > table > tbody > tr > td > table > tbody > tr > td > div:nth-child(5) > table > tbody > tr:nth-child('+str(
                            num + 1)+') > td:nth-child(5) > div.decBt02 > a'
                        self.driver.find_element_by_css_selector(
                            selector).click()
